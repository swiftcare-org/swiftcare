...
- `QueueManagementPage` (Receptionist) — displays the current clinic day's full queue and polls every five seconds. It resolves names from PatientService by `PatientId`, caches successful lookups across polls, and loads prescription state for completed entries. A pending prescription links to `View Prescription`; a dispensed prescription shows `✅ DISPENSED` and remains available for viewing.
...
- `PrescriptionPage` (Doctor) — opens after consultation completion succeeds, shows advisory allergy warnings and previous prescriptions, accepts multiple medicine items, and saves a `PENDING` prescription linked to the consultation, queue, patient, and authenticated doctor. After saving, the doctor can add medicines or remove a confirmed item while keeping at least one; a `DISPENSED` prescription is read-only. If redirect state is unavailable after a refresh or new browser context, it recovers the latest unfinished prescription context from backend data.
- `PrescriptionDetailsPage` (Doctor, Receptionist, Admin) — loads a prescription and its ordered medicines by queue entry. Receptionists can mark a `PENDING` prescription as dispensed once. A dispensed prescription shows the receptionist and the dispensing time in the clinic timezone, while Doctor and Admin access remains read-only.
